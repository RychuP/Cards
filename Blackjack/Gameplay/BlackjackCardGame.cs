...
using Blackjack.Gameplay.Rules;
...
class BlackjackCardGame : CardGame
{
    ...
    public BlackjackCardGame(Rectangle tableBounds, Vector2 dealerPosition,
        ...
        : base(6, 0, CardSuits.AllSuits, CardValues.NonJokers, Fonts.Moire.Regular, MinPlayers, MaxPlayers,
    ...
    private void ShowShuffleAnimation()
    {
        ...
        animationComponent.AddAnimation(
            new FramesetGameComponentAnimation(CardAssets.ShuffleSpriteSheets[Theme], 32, 11, _frameSize)
            {
    ...
    private void ShowDealerValue()
    {
        ...
        // Draw the value
        Vector2 measure = Fonts.Moire.Regular.MeasureString(dealerValue);
        Vector2 position = GameTable.DealerPosition - new Vector2(measure.X + 20, 0);

        sb.Draw(Art.Blank, new Rectangle((int)position.X - 4, (int)position.Y,
            (int)measure.X + 8, (int)measure.Y), Color.Black);

        sb.DrawString(Fonts.Moire.Regular, dealerValue,
    ...
    private void DrawValue(AnimatedHandGameComponent animatedHand, int place, string value, Color valueColor)
    {
        ...
        Vector2 measure = Fonts.Moire.Regular.MeasureString(value);

        position.X += (CardAssets.CardBacks[Theme].Bounds.Width - measure.X) / 2;
        position.Y -= measure.Y + 5;

        sb.Draw(Art.Blank, new Rectangle((int)position.X - 4, (int)position.Y,
            (int)measure.X + 8, (int)measure.Y), Color.Black);
        sb.DrawString(Fonts.Moire.Regular, value, position, valueColor);
    ...
    void CueOverPlayerHand(BlackjackPlayer player, Texture2D asset,
        HandTypes animationHand, AnimatedHandGameComponent waitForHand)
    {
        ...
        AnimatedGameComponent animationComponent = new(this, asset)
        {
    ...
    private void ShowResultForPlayer(BlackjackPlayer player, int dealerValue, HandTypes currentHandType)
    {
        ...
        {
            Texture2D asset = GetResultAsset(player, dealerValue, playerValue);
            CueOverPlayerHand(player, asset, currentHandType, _dealerHandComponent);
        }
    }
    ...
    private Texture2D GetResultAsset(BlackjackPlayer player, int dealerValue, int playerValue)
    {
        Texture2D asset;
    ...
    private void DealerAI()
    {
        ...
        if (dealerValue > 21)
        {
            ...
            CueOverPlayerHand(_dealerPlayer, Art.Bust, HandTypes.First, _dealerHandComponent);
        }
        else if (dealerValue == 21)
        {
            _dealerPlayer.BlackJack = true;
            CueOverPlayerHand(_dealerPlayer, Art.Blackjack, HandTypes.First, _dealerHandComponent);
        }
    ...
    private void EndGame()
    {
        ...
        // as a timer.
        AnimatedGameComponent animationComponent = new(this, Art.YouLoose)
        {
            Position = new Vector2(
                Game.GraphicsDevice.Viewport.Bounds.Center.X - Art.YouLoose.Width / 2,
                Game.GraphicsDevice.Viewport.Bounds.Center.Y - Art.YouLoose.Height / 2),
            Visible = false
        };
        Game.Components.Add(animationComponent);
    ...
    public void ShowPlayerPass(int indexPlayer)
    {
        ...
        AnimatedGameComponent passComponent = new(this, Art.Pass)
        {
    ...
    void BustRule_OnMatch(object sender, EventArgs e)
    {
        ...
        CueOverPlayerHand(player, Art.Bust, args.Hand, null);
    ...
    void BlackJackRule_OnMatch(object sender, EventArgs e)
    {
        ...
        CueOverPlayerHand(player, Art.Blackjack, args.Hand, null);
